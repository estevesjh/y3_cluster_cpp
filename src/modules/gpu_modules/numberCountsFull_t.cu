...
//integrating over (lambda_tr, z, lnM). The lambda_ob integral is done analytically via
// the EMG CDF, matching the template pattern used for photo-z bins.
//
// Integrand:
//   N = int dltr int dz int dlnM
//       Omega(z) * (dV/dOmega/dz)(z) * n(M,z)
//       * P_HOD(ltr|M,z) * P_EMG_bin(lob_low,lob_high|ltr,z) * K(zo|z)
//
// where:
//   - P_HOD(ltr|M,z) is the MOR (mass-observable relation): mor_des_log_t
//   - P_EMG_bin = CDF(lob_high|ltr,z) - CDF(lob_low|ltr,z) is the richness bin kernel
...
// numberCountsFull_t: 3D integration over (ltr, zt, lnM) with lob
// integrated analytically via EMG CDF.
...
private:
  ...
  // Models from datablock (using std::optional for deferred initialization)
...
public:
  ...
  // No special configuration needed at setup time for this module.
  ...
  // The integrand function called at each (ltr, zt, lnM) point.
  ...
  operator()(double ltr, double zt, double lnM) const
  {
    ...
    // Richness bin kernel: integral of P(lob | ltr, z) over [lob_low, lob_high]
    // Using CDF: P(lob_low < lob < lob_high | ltr, z) = CDF(lob_high) - CDF(lob_low)
    // fmax guards against tiny negative values from floating-point subtraction
    double const richness_bin = fmax(0.0, emg_->cdf(lob_high_, ltr, zt) - emg_->cdf(lob_low_, ltr, zt));
    if (richness_bin == 0.0) {
      return 0.0;
    }
