...
namespace y3_cluster {
    ...
    dsigma_table_file(std::string const& kernel)
    {
      return kernel_is_signed(kernel)
        ? fmt::format(
            ...
            kernel)
        : fmt::format(
            "nfw_off_center/table_1000_1e-03_5e+03_log_deltasigma_{}.txt",
            kernel);
    }
  ...
  class NFW_DSIGMA_MIS {
    ...
    operator()(double r, double rmis, double lnM) const
    {
      ...
      // "single" table is SIGNED/linear (may be < 0 for xmis > x); the legacy
      // "gamma" table is log, so exponentiate only in that case.
