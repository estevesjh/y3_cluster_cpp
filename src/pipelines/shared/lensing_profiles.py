...
class NfwDsigmaMisProduction:
    """NFW_DSIGMA_MIS replica: bilinear in (ln x_mis, ln x), clamped.

    Mirrors nfw_dsigma_mis.hh's kernel_is_signed contract: the "single"
    kernel reads the SIGNED (linear) table
    table_1000_..._deltasigma_signed_single.txt (regenerated by
    data/nfw_off_center/make_signed_deltasigma_table.py) and must NOT be
    exponentiated -- the negative lobe at r_mis > r is physical and is
    what makes the mean-field ("rnd") projection term cancel. Only the
    ...
    def __init__(self, kernel="gamma", data_dir=None):
        ...
        table_name = (
            f"table_1000_1e-03_5e+03_deltasigma_signed_{kernel}.txt"
            if self._is_signed
            else f"table_1000_1e-03_5e+03_log_deltasigma_{kernel}.txt")
        tab = np.loadtxt(d / table_name)
        self._interp = RegularGridInterpolator(
