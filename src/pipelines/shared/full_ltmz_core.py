...
def full_ltmz_mass_z_weights(bins, mor, plob_splines, hmf, dv, *,
                             sci=None, zt_low, zt_high, lnm_low, lnm_high,
                             n_lnm=96, n_z=64, n_q=32, l_lam=6.0,
                             lnm_nodes=None):
    """z-RESOLVED full_ltmz weights: W2d[b, k, q] before the z sum.
    ...
    reproduces full_ltmz_mass_weights exactly.
    """
    sf = sel_kernels.load()
    z_x, z_w = dm.gl_nodes(zt_low, zt_high, n_z)
    if lnm_nodes is None:
