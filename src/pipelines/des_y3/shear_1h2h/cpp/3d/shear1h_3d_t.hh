// Miscentred one-halo shear — explicit (lambda_true, lnM, z) 3d adaptive reference, C++.
//
// The brute-force adaptive counterpart of the Python explicit-GL shear
// backend (../python/shear1h_explicit_gl.py): one Cuhre triple integral
// per (bin, R) point,
//
...
//
// Configuration: the counts 3d options (bin definitions +
...
//
// Output: shear1h3d/{vals, errors, probs, status, nregions}.
// Status: reference backend. Production remains Shear1hMisSel.so.
#ifndef Y3_CLUSTER_CPP_SHEAR1H_3D_T_HH
#define Y3_CLUSTER_CPP_SHEAR1H_3D_T_HH
...
public:
  explicit Shear1h3d(cosmosis::DataBlock& cfg)
  {
    ...
    if (lam_max.size() != n || zob_min_.size() != n ||
      ...
      throw std::runtime_error(
        "Shear1h3d: bin definition arrays have unequal lengths");
    s_i_.reserve(n);
    ...
    if (lob_centers_.empty())
      throw std::runtime_error("Shear1h3d: lob_centers is empty");
  }
  ...
  set_grid_point(grid_point_t const& pt)
  {
    ...
    if (current_bin_ < 0 ||
      ...
      throw std::out_of_range(
        "Shear1h3d: bin_index outside the configured bin set");
    current_r_mis_ =
