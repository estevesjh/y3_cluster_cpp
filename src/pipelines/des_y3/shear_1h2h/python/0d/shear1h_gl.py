"""Miscentred one-halo shear via the S_ij-tabulated fixed-GL path (formerly fast_mass) — Python.

The exact-redshift-contraction fast path for the shear observable,
extending the plan's `fixed-GL` strategy from counts to shear at the
...
explicit-3d, it consumes the tabulated S_ij rather than re-evaluating the
selection kernels.
...
Writes: shear1h_gl/vals  (n_bins * n_r,)   [hardcoded section]
