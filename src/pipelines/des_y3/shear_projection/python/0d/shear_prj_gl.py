"""Projection shear via the S_ij-tabulated fixed-GL path (formerly fast_mass) — Python (exact z).
...
integral done exactly, outside the radial operator — the fixed-GL
...
with the exact per-slice redshift weights
    ...
    common(z)     = dV/dOmega/dz(z) w_pz(z; z_ob) w_z^GL
...
Writes (hardcoded sections):
    dsigma_prj_gl/{vals,rnd,cl}   Msun/(h pc^2), length 180
    shear_prj_gl/{vals,rnd,cl}    dimensionless, length 180
...
DSIGMA_SECTION = "dsigma_prj_gl"
SHEAR_SECTION = "shear_prj_gl"
...
class ShearPrjGl:
    ...
    def set_sample(self, source):
        ...
        for s in self.slices:
            ...
            # Exact z contraction (the fixed-GL step).
            chi_z = chi(zs)
            sig_z = z_kernel.sigma_z(zs)
            u_phot = (zs - zob) / sig_z
            w_pz = np.where(np.abs(u_phot) < 1.0, 1.0 - u_phot**2, 0.0)
            common = dv(zs) * wzs * w_pz                     # (nz,)
...
def setup(options):
    ...
    for key, default in (("zt_low", 0.10), ("zt_high", 0.75),
        ...
        try:
            ...
            knobs[key] = default
    return ShearPrjGl(wall, lob_centers=lob_centers, **knobs)
...
def execute(block, core):
    ...
    print(f"[shear_prj_gl] {core.wall_n} wall points, "
