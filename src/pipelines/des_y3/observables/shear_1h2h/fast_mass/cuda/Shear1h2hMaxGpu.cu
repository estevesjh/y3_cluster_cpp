...
//
// A port of the des_y3 fast_mass C++ backend (Shear1h2hMax.cc): same
// observable,
//
...
// modules pass their integrands); the (lnM, z) contraction against the
...
// compute_lensing_2h = T (same haloModel contract as Shear1h2hMax.cc).
...
namespace {
  ...
  // partial sum into the (bin, R) accumulator.
  __global__ void
  max_model_contract(y3_cuda::NFW_DSIGMA_MIS nfw, double rho_mult,
                      ...
                      double* acc)
  {
    ...
    double const d_mis = rho_mult * nfw(R, r_mis[b], lnm_x[k]);
...
public:
  ...
  explicit Shear1h2hMaxGpu(cosmosis::DataBlock& cfg)
    ...
    , include_mis_(cfg.has_val(module_label(), "include_miscentering")
                     ...
                     : true)
    , dsigma_mis_dev_(4.0, 2.77533742639e+11, y3_cuda::GAMMA)
  {
  ...
  set_sample(cosmosis::DataBlock& s)
  {
    ...
    double const f_mis_scalar =
      include_mis_
        ? w::mis_detail::read_mis_param(s, "f_mis", w::mis_detail::F_MIS_DEFAULT)
        : 0.0;
    double const tau_mis = w::mis_detail::read_mis_param(
      s, "tau_mis", w::mis_detail::TAU_MIS_DEFAULT);
    double const omega_M = s.view<double>("cosmological_parameters", "omega_M");

    // z-only factors (Sigma_crit_inv folded in) and the z-RESOLVED
    // weight W2d(bin; lnM, z) -- identical construction to
    // Shear1h2hMax.cc, host-side (not the bottleneck: O(n_bins*N_lnm*N_z)
    ...
    for (std::size_t b = 0; b != n_bins_; ++b)
      r_mis[b] = tau_mis * w::mis_detail::R_lambda(
    ...
    max_model_contract<<<blocks, threads>>>(
      dsigma_mis_dev_, omega_M, int(n_bins_), int(n_R), int(N_lnm_),
