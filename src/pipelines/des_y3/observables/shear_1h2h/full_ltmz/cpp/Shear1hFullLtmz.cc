...
//             Σ_crit^-1(zt) K_j(zt) K_i(lt, zt) P_HOD(lt | M, zt)
//             Φ_i(R, lnM),
//
// with Φ_i the production miscentred mixture ((1 - f_mis) haloModel
// ΔΣ_nfw + f_mis gamma-table ΔΣ_mis, Ω_m amplitude). All physics terms
// are existing immutable models; the module instantiates the immutable
// scalar-integration template. Deliberately makes no use of Φ being
// z-free — the whole point of this backend is an integration strategy
// with no shared structure with the fixed-GL references it validates.
//
...
private:
  ...
  y3_cluster::NFW_DSIGMA_MIS dsigma_mis_{4.0, 2.77533742639e+11,
                                         y3_cluster::GAMMA};
...
public:
  ...
  set_sample(cosmosis::DataBlock& sample)
  {
    ...
    f_mis_ = w::mis_detail::read_mis_param(sample, "f_mis",
                                           w::mis_detail::F_MIS_DEFAULT);
    tau_mis_ = w::mis_detail::read_mis_param(sample, "tau_mis",
  ...
  operator()(double lt, double zt, double lnM) const
  {
    ...
    return (*hmf_)(lnM, zt) * (*dv_do_dz_)(zt) * (*omega_z_)(zt) *
           sci_->clamp(zt) * k_j * k_i_[current_bin_](lt, zt, *plob_) *
           (*mor_)(lt, lnM, zt) * phi;
  }
...
DEFINE_COSMOSIS_SCALAR_INTEGRATION_MODULE(Shear1hFullLtmz)
