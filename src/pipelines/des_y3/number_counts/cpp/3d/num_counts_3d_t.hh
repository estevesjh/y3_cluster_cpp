// Full (lambda_true, lnM, z) reference number counts — explicit 3d adaptive (formerly `explicit-3d`), C++.
//
...
//
// This is the C++ backend of the des_y3 explicit 3d strategy
...
//
// Output: numcounts3d/{vals, errors, probs, status, nregions},
// one entry per bin.
//
// Status: reference backend (validated against the Python explicit-GL
// reference and production NumCountsSel.so; see README.md). Not a
// production entry point.
#ifndef Y3_CLUSTER_CPP_NUM_COUNTS_3D_T_HH
#define Y3_CLUSTER_CPP_NUM_COUNTS_3D_T_HH
...
public:
  explicit NumCounts3d(cosmosis::DataBlock& cfg)
  {
    ...
    if (lam_max.size() != n || zob_min_.size() != n ||
      ...
      throw std::runtime_error(
        "NumCounts3d: bin definition arrays have unequal lengths");
    s_i_.reserve(n);
  ...
  set_grid_point(grid_point_t const& pt)
  {
    ...
    if (current_bin_ < 0 ||
      ...
      throw std::out_of_range(
        "NumCounts3d: bin_index outside the configured bin set");
  }
