"""Number counts via the S_ij-tabulated fixed-GL path (formerly fast_mass) — Python backend.

The plan's `fixed-GL` strategy made explicit: the redshift integral is
...
importable statement of the production algorithm that the explicit 3d
...
Writes: numcounts_sij_gl/vals  (len(bin_index),)  [hardcoded section]
...
OUTPUT_SECTION = "numcounts_sij_gl"
...
def execute(block, cfg):
    ...
    print(f"[numcounts_sij_gl] {len(bins)} bins "
