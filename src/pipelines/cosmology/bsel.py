...
Ported into ``src/pipelines/cosmology`` (see README.md in this
directory) from ``y3_buzzard/bsel.py``, whose only functional change
here is the ``PrjParams`` import below, pointed at the copy that now
lives alongside this file instead of ``y3_buzzard``. The ``y3_buzzard``
original stays in place and is unaffected by this port.
...
Matches the Python reference in ``richness_selection.sel_bias.SelBias``:
  ...
  * ltr marginalisation weighted by the EMG plob_ltr kernel
    ...
    :class:`cosmology.prj_params.PrjParams` (the canonical Y3 fit baked
...
_COSMOLOGY_DIR = str(Path(__file__).resolve().parent.parent)
if _COSMOLOGY_DIR not in sys.path:
    sys.path.insert(0, _COSMOLOGY_DIR)
