...
// (src/pipelines/des_y3/shear_projection/cpp/0d/
// ShearPrjGl.cc).
//
// ShearPrjGl.cc has no fast_mass-specific header of its own -- it is
...
// (../python/shear_prj_gl.py), which the module's docstring
...
TEST_CASE("sigma_prj theta grid (build_theta_grid) is bounded, monotone, and matches the Python fast_mass port")
{
  ...
  // Golden values from ../python/shear_prj_gl.py::build_theta_grid
...
TEST_CASE("sigma_prj LoS-slab exclusion angle (theta_excl_at_z) matches the Python fast_mass port")
{
  ...
  // ../python/shear_prj_gl.py::theta_excl_at_z.
...
{
  // The config contract ShearPrjGl.cc relies on: ShearPrjCore's
  ...
  // ShearPrjGl.cc instantiates; it stops short of set_sample()
  // (see file header) since that needs a full pipeline sample.
  cosmosis::DataBlock cfg;
  char const* section = "ShearPrjGlTest";
  ...
  // same construction cost ShearPrjGl pays.
