...
#include "pipelines/des_y3/observables/shear_projection/full_ltmz/cuda/DSigmaPrjFullLtmzGpu.cu"
...
namespace {
  ...
  __global__ void
  eval_kernel(DSigmaPrjFullLtmzGpu obj, double lntheta, double zt,
  ...
  double
  eval_on_device(DSigmaPrjFullLtmzGpu const& obj, double lntheta, double zt,
                double lnM)
  {
...
TEST_CASE("DSigmaPrjFullLtmzGpu integrand is exactly zero outside the photo-z window")
{
...
TEST_CASE("DSigmaPrjFullLtmzGpu integrand matches its documented formula assembly")
{
  cosmosis::DataBlock db = make_sample();
  DSigmaPrjFullLtmzGpu integrand(db);
  integrand.set_sample(db);
  ...
  double const zt = zob;         // u = 0 exactly -> w_phot = 1 exactly,
  ...
  double const expected = theta * 2.0 * M_PI * std::sin(theta) * dv_do_dz
                         * w_phot * hmf * (1.0 + cl) * omega_m * dsmis;
