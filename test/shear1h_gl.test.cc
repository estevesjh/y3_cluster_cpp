...
// src/pipelines/des_y3/shear_1h2h/cpp/0d/Shear1hGl.cc
//
// Shear1hGl composes two immutable production pieces under a new
// module label / output section (shear1h_gl/vals): the header-only
...
//   1. Shear1hGl::evaluate() against an independently assembled
...
//      that is actually new in Shear1hGl.cc.
//   2. Shear1hGl against y3_cluster::Shear1hMisSelGL (the
...
//
// ASSUMPTION flagged for the coordinator: Shear1hGl is defined
...
// The header-only model machinery Shear1hGl.cc composes: SelGLCore
...
#include "pipelines/des_y3/shear_1h2h/cpp/0d/shear1h_gl_t.hh"
...
namespace {
  ...
  // Options section shared by Shear1hGl (the system under test) and
  ...
  make_cfg_block(std::vector<double> const& lob_centers)
  {
    ...
    for (char const* label : {"Shear1hGl", "Shear1hMisSel"}) {
  ...
  // Shear1hGl compose, per the W_ij(lnM)/mixture formula
...
TEST_CASE("Shear1hGl matches an independently assembled W_ij x mixture reference")
{
...
TEST_CASE("Shear1hGl agrees with the production-equivalent Shear1hMisSelGL")
{
...
TEST_CASE("Shear1hGl at f_mis = 0 is invariant to the miscentering-kernel parameters")
{
  ...
  Shear1hGl mod_a(cfg_a);
  mod_a.set_sample(s);
  Shear1hGl mod_b(cfg_b);
...
TEST_CASE("Shear1hGl guard clauses: empty lob_centers, out-of-range bin_index")
{
  cosmosis::DataBlock cfg_empty;
  cfg_empty.put_val("Shear1hGl", "n_lnm", N_LNM);
  cfg_empty.put_val("Shear1hGl", "n_z", N_Z);
  cfg_empty.put_val("Shear1hGl", "zt_low", ZT_LO);
  cfg_empty.put_val("Shear1hGl", "zt_high", ZT_HI);
  cfg_empty.put_val("Shear1hGl", "lnm_low", LNM_LO);
  cfg_empty.put_val("Shear1hGl", "lnm_high", LNM_HI);
  cfg_empty.put_val("Shear1hGl", "lob_centers", std::vector<double>{});
  CHECK_THROWS_AS(Shear1hGl(cfg_empty), std::runtime_error);
