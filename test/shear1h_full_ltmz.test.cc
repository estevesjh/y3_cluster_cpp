...
// (src/pipelines/des_y3/observables/shear_1h2h/full_ltmz/cpp,
...
// mixture profile.
//
// Shear1hFullLtmz has no committed numeric fiducial corpus to pin
...
// section Shear1hFullLtmz::set_sample reads, and checks operator()
// against those same sub-models assembled by hand per the
// header-documented O_ij(R) integrand. That is an algorithm-identity
// check in CLAUDE.md's sense: it pins that Shear1hFullLtmz composes the
...
#include "pipelines/des_y3/observables/shear_1h2h/full_ltmz/cpp/Shear1hFullLtmz.cc"
...
namespace {
  ...
  // Populate every DataBlock section Shear1hFullLtmz's constructor and
  ...
  {
    std::string const label = "Shear1hFullLtmz";
...
TEST_CASE("Shear1hFullLtmz reproduces the documented O_ij integrand from its own production sub-models")
{
  cosmosis::DataBlock cfg;
  double const f_mis = 0.22, tau_mis = 0.17;
  populate(cfg, f_mis, tau_mis);

  Shear1hFullLtmz model(cfg);
  model.set_sample(cfg);
  model.set_grid_point({0.0, R_Q});

  // Independently assemble the same terms Shear1hFullLtmz::operator()
...
TEST_CASE("Shear1hFullLtmz miscentering mixture is affine in f_mis, anchored to the production dSigma_nfw/NFW_DSIGMA_MIS readers")
{
  cosmosis::DataBlock cfg0;
  populate(cfg0, /*f_mis=*/0.0, /*tau_mis=*/0.17);
  Shear1hFullLtmz centred(cfg0);
...
TEST_CASE("Shear1hFullLtmz falls off monotonically with radius")
{
  cosmosis::DataBlock cfg;
