...
| nersc **#2** z-bin edges/seam | **Done** (`ac85b33`; issue premise confirmed by the owner against the authoritative DV source): the production DV is Tan Xing's notebook (xtang126/mock_cluster_buzzard, cell 3: `ZMIN_LIST=[0.20,0.37,0.50]`, `ZMAX_LIST=[0.33,0.50,0.65]` on scattered z_obs) — seam-excluding observed edges restored in `mock_mcmc_cp_camb_buzzard.ini`. Caution: the notebook's markdown header AND this repo's `build_buzzard_datavector.py` both show stale 0.35 edges — neither is production. Residual for y3#8: true-z hole (σ_z-order edge leakage) + Ω(z). | 1 |
