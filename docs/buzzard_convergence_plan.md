...
| y3 **#10** wall-metadata contract | **Fix now — blocks #11.** Recommended option: promote the harness shim (`common/bsel_wall_meta.py`) to a permanent pipeline module in `src/pipelines/shared/` (option 2): no touch to the immutable C++ evaluator template, works for both refactored consumers, and makes `real_pipeline_extract_prj2h.ini` regenerate the projection dump sections. | 1 |
...
| nersc **#2** z-bin edges/seam | **Quick fix now:** set pipeline z-edges to ZMIN=[0.20,0.37,0.50] / ZMAX=[0.33,0.50,0.65] in every z-binned section of `mock_mcmc_cp_camb.ini`. Cheap, kills the dominant NC deficit; the true-z-side excision then rides on y3#8. | 1 |
